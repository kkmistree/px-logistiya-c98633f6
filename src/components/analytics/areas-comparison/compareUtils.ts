...
export const isLocation1Higher = (value1: string, value2: string, type: string) => {
  if (type === 'volume') {
    ...
    return parseFloat(value1.replace('AED ', '').replace('SAR ', '').replace('B', '')) > parseFloat(value2.replace('AED ', '').replace('SAR ', '').replace('B', ''));
  } else if (type === 'price-sqft' || type === 'price') {
    return parseFloat(value1.replace('AED ', '').replace('SAR ', '').replace(/,/g, '')) > parseFloat(value2.replace('AED ', '').replace('SAR ', '').replace(/,/g, ''));
  }
  return false;
};
